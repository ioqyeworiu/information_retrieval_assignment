...
class ExerciseDocRoute:
    ...
    @exercise_doc_router.post(f"/api/public/v2/excercise_docs")
    def get_exercise_doc_v2(credentials: int = Depends(get_credentials), query_params: Query = Depends(), query: str = Body(), exercise_doc_service: ExerciseDocService = Depends()):
        return {"data": exercise_doc_service.get_exercise_doc(credentials, query_params, query)}